...
const getCrawlDependencies = async () => {
  const [
    { ProxyManager },
    { BarnesNobleScraper },
    {
    ...
    if (process.env.NODE_ENV === 'production' && !process.env.NEXT_PUBLIC_SUPABASE_URL) {
      return NextResponse.json({
        ...
        data: {
          isRunning: false,
          progress: null,
          logs: ['Build mode - crawl service unavailable']
        }
      });
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (process.env.NODE_ENV === 'production' && !process.env.NEXT_PUBLIC_SUPABASE_URL) {
      return NextResponse.json({
        success: false,
        error: 'Crawl service unavailable during build'
      }, { status: 503 });
    }
    ...
    const { getCategoryById } = await getCrawlDependencies();
...
async function startLinksCrawl(category: any, options: any) {
  ...
  try {
    // Lazy load dependencies
    const { ProxyManager, BarnesNobleScraper, updateCategoryStatus, createProductLink } = await getCrawlDependencies();
    ...
    // Lazy load dependencies for error handling
    const { updateCategoryStatus: updateStatus } = await getCrawlDependencies();
    await updateStatus(category.id, 'error');
  } finally {
    crawlState.isRunning = false;
...
async function startProductsCrawl(category: any, options: any) {
  ...
  try {
    // Lazy load dependencies
    const { ProxyManager, BarnesNobleScraper, updateCategoryStatus, getProductLinks, createProduct, updateProductLink } = await getCrawlDependencies();
